...
module Inferno
  module Sequence
    class DemoCancerstageparentSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Cancer Stage Parent Tests'
      description 'Verify support for the server capabilities required by the Cancer Stage Parent profile.'
      details %(
      )
      test_id_prefix 'CSP'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-stage-parent'
          description %(
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-stage-parent')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Observation resource that matches the Cancer Stage Parent profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-stage-parent'
          description %(
            This test will validate that the Observation resource returned from the server matches the Cancer Stage Parent profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-stage-parent')
      end
    end
  end
end
