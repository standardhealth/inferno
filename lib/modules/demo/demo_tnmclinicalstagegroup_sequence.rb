...
module Inferno
  module Sequence
    class DemoTnmclinicalstagegroupSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'TNM Clinical Stage Group Tests'
      description 'Verify support for the server capabilities required by the TNM Clinical Stage Group profile.'
      details %(
      )
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tnm-clinical-stage-group'
          description %(
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tnm-clinical-stage-group')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Observation resource that matches the TNM Clinical Stage Group profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tnm-clinical-stage-group'
          description %(
            This test will validate that the Observation resource returned from the server matches the TNM Clinical Stage Group profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tnm-clinical-stage-group')
      end
    end
  end
end
