...
module Inferno
  module Sequence
    class DemoTumormarkerSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Tumor Marker Tests'
      description 'Verify support for the server capabilities required by the Tumor Marker profile.'
      details %(
      )
      test_id_prefix 'TM'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tumor-marker'
          description %(
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tumor-marker')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Observation resource that matches the Tumor Marker profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tumor-marker'
          description %(
            This test will validate that the Observation resource returned from the server matches the Tumor Marker profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-tumor-marker')
      end
    end
  end
end
