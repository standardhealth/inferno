...
module Inferno
  module Sequence
    class DemoPrimarycancerconditionSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Primary Cancer Condition Tests'
      description 'Verify support for the server capabilities required by the Primary Cancer Condition profile.'
      details %(
      )
      test_id_prefix 'PCC'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-primary-cancer-condition'
          description %(
        ...
        save_resource_references(versioned_resource_class('Condition'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-primary-cancer-condition')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Condition resource that matches the Primary Cancer Condition profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-primary-cancer-condition'
          description %(
            This test will validate that the Condition resource returned from the server matches the Primary Cancer Condition profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Condition', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-primary-cancer-condition')
      end
    end
  end
end
