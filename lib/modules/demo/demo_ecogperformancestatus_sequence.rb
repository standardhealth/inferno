...
module Inferno
  module Sequence
    class DemoEcogperformancestatusSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'ECOG Performance Status Tests'
      description 'Verify support for the server capabilities required by the ECOG Performance Status profile.'
      details %(
      )
      test_id_prefix 'ECOGPS'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-ecog-performance-status'
          description %(
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-ecog-performance-status')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Observation resource that matches the ECOG Performance Status profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-ecog-performance-status'
          description %(
            This test will validate that the Observation resource returned from the server matches the ECOG Performance Status profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-ecog-performance-status')
      end
    end
  end
end
