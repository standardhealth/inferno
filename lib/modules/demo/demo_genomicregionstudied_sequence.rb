...
module Inferno
  module Sequence
    class DemoGenomicregionstudiedSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Genomic Region Studied Tests'
      description 'Verify support for the server capabilities required by the Genomic Region Studied profile.'
      details %(
      )
      test_id_prefix 'GRS'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-genomic-region-studied'
          description %(
        ...
        save_resource_references(versioned_resource_class('Observation'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-genomic-region-studied')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Observation resource that matches the Genomic Region Studied profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-genomic-region-studied'
          description %(
            This test will validate that the Observation resource returned from the server matches the Genomic Region Studied profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Observation', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-genomic-region-studied')
      end
    end
  end
end
