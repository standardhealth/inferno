...
module Inferno
  module Sequence
    class DemoCancerconditionparentSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Cancer Condition Parent Tests'
      description 'Verify support for the server capabilities required by the Cancer Condition Parent profile.'
      details %(
      )
      test_id_prefix 'CCP'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-condition-parent'
          description %(
        ...
        save_resource_references(versioned_resource_class('Condition'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-condition-parent')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Condition resource that matches the Cancer Condition Parent profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-condition-parent'
          description %(
            This test will validate that the Condition resource returned from the server matches the Cancer Condition Parent profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Condition', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-cancer-condition-parent')
      end
    end
  end
end
