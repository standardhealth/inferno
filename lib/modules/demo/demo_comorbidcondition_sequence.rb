...
module Inferno
  module Sequence
    class DemoComorbidconditionSequence < SequenceBase
      include Inferno::SequenceUtilities

      title 'Comorbid Condition Tests'
      description 'Verify support for the server capabilities required by the Comorbid Condition profile.'
      details %(
      )
      test_id_prefix 'CC'
      ...
      test :resource_read do
        metadata do
          ...
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-comorbid-condition'
          description %(
        ...
        save_resource_references(versioned_resource_class('Condition'), [@resource_found], 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-comorbid-condition')
      end
      ...
      test :resource_validate_profile do
        metadata do
          id '03'
          name 'Server returns Condition resource that matches the Comorbid Condition profile'
          link 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-comorbid-condition'
          description %(
            This test will validate that the Condition resource returned from the server matches the Comorbid Condition profile.
          )
          versions :r4
        end

        skip 'No resource found from Read test' unless @resource_found.present?
        test_resources_against_profile('Condition', 'http://hl7.org/fhir/us/mcode/StructureDefinition/mcode-comorbid-condition')
      end
    end
  end
end
